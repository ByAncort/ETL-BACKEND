...
def _to_native(obj: Any) -> Any:
    """Convierte numpy types a Python nativos recursivamente."""
...
def predict(req: PredictRequest):
    ...
    """
    predictor = get_predictor()
    result = _to_native(predictor.predecir(req.field_a, req.field_b))
...
def predict_batch(req: PredictBatchRequest):
    ...
    for field_a, field_b in req.pairs:
        result = _to_native(predictor.predecir(field_a, field_b))
        resultados.append({
            "campo1": field_a,
            "campo2": field_b,
            "son_similares": result["son_similares"],
...
def predict_objects(req: PredictObjectsRequest):
    ...
    predictor = get_predictor()
    result = _to_native(predictor.predecir_objetos(req.object_a, req.object_b, estrategia=req.strategy))
